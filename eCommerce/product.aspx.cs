...
public partial class product : System.Web.UI.Page
{
    ...
    protected void btnBuy_Click(object sender, EventArgs e)
    {
        ...
        if (Session["CustomerID"] == null)
        {
            string redirectToURL = "~/ProductCollection.aspx?addToCart=";
        ...
        {
            string ProductID = Request.QueryString["ProductID"];
            Response.Redirect("~/ProductCollection.aspx?addToCart=" + ProductID + notification);
        }
    }
    ...
    {
        string notifyTitle = "Added to Wish List";
        string message = "One new item has been added to wish list.";
        string notification = string.Format("&?notifyTitle={0}&notificationDescription={1}", notifyTitle, message);

        if (Session["CustomerID"] == null)
        {
            string redirectToURL = "~/ProductCollection.aspx?addToWishlist=";
            string ProductID = Request.QueryString["ProductID"];
            Response.Redirect("~/login.aspx?redirect=" + redirectToURL + ProductID);
        }
        else
        {
            string ProductID = Request.QueryString["ProductID"];
            Response.Redirect("~/ProductCollection.aspx?addToWishlist=" + ProductID + notification);
        }
